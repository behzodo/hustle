...
/**
 * A profile, as the browser gets one.
 *
 * Spread from the schema rather than restated. A `returns` validator is
 * exact, and this list was written out by hand — which is how the same
 * mistake reached `leads` twice, each time taking a screen down for every
 * row that had the new column on it.
 */
