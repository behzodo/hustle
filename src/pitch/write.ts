...
import { pitchTone, priceNote, PITCH_FORMAT, PITCH_SYSTEM } from "./prompt";
import { PitchDraftSchema, type PitchDraft } from "./schema";

/**
 * Writing one email, and refusing to accept a bad one.
 *
 ...
 * does, and "mostly" over four hundred businesses is a number of emails that
 ...
 * fails. A page with a bad line can be published and fixed; an email cannot be
...
export interface PitchFacts {
  ...
  town?: string;
  categories?: string[];
  /** Their own site, freshly built and already live. The point of the email. */
  siteUrl: string;
}

export interface Sender {
  /** Signs the email. Required — an unsigned cold email is an anonymous one. */
  tradingName: string;
  /** Where they work, for the line under the name. */
  city?: string;
  /** Tone slug from the profile. */
  tone?: string;
  /** Price band slug. Absent means the email may not mention money at all. */
  priceBand?: string;
}

export interface ComposedPitch {
  subject: string;
  /** The whole email, sign-off included, ready to send. */
  body: string;
 ...
 *
 * Three things have to be in it and none of them may be paraphrased: who sent
 * it, where they are, and how to make it stop. A model asked for a sign-off
 * will sometimes give a warm one and sometimes give a witty one and
 * occasionally give somebody else's name.
 *
 * The last line is the opt-out. A cold commercial email needs a working way to
 * refuse more of them — that is the law in most places this will be sent, and
 * it is also just the decent version of doing this. "Reply and it stops" is a
 * mechanism a person can actually use, unlike a link to a page nobody trusts.
 *
 * What is deliberately not here is a postal address, which CAN-SPAM also
 * requires and the profile does not currently hold. See the note in
 * src/pitch/index.ts.
 */
...
const describe = (facts: PitchFacts, sender: Sender) => {
  ...
  const lines = [
    ...
    price
      ? `What they charge, if the email mentions money at all: ${price}`
...
export const writePitch = async (
  ...
  for (let attempt = 0; attempt <= MAX_REWRITES; attempt++) {
    ...
    try {
      draft = await askJson(PitchDraftSchema, {
        system: PITCH_SYSTEM,
        user: complaint
          ? `${user}\n\nYour last attempt was rejected because it ${complaint}. Write it again without that.`
          : user,
        maxTokens: 2000,
        // Obedience, not reasoning — the same call the brief makes, for the same
        // reason. The rules are explicit and the output is two short fields.
        thinking: "low",
        ...
        // email is one that gets forwarded between them.
        temperature: 0.9,
        order: options.order,
      });
    } catch (cause) {
