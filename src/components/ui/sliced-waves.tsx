...
/**
 * Animated WebGL bars that slide against each other — a moving backdrop.
 *
 * The props are pushed straight onto the shader's uniforms in a second effect
 * rather than rebuilding the renderer, so retinting or reshaping it mid-scene
 * costs nothing and never drops a frame.
 */
export const SlicedWaves = ({
  ...
  useEffect(() => {
    ...
    // Scrolled out of view or on a background tab, the shader is doing work
    // nobody can see — both observers exist to stop the loop, not to start it.
  ...
  useEffect(() => {
    ...
    (
      [
        [u.uColor1.value as Float32Array, color1],
        [u.uColor2.value as Float32Array, color2],
        [u.uColor3.value as Float32Array, color3],
      ] as const
    ).forEach(([target, hex]) => target.set(hexToRgb(hex)));
  }, [
    color1,
    color2,
    color3,
