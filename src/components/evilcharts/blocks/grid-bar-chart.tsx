...
// Parameterised so the same grid treatment can carry different series. The
// registry ships it hardcoded to a demo dataset; the defaults below keep that
// behaviour for anyone rendering it with no props.
interface EvilGridBarChartProps {
...
export function EvilGridBarChart({
  ...
  totalLabel = "[Σ] Total",
  peakLabel = "[⬆] Peak",
}: EvilGridBarChartProps = {}) {
  const series = data ?? chartData;
  const total = series.reduce((sum, item) => sum + item.desktop, 0);
  const maxData = series.reduce(
    (max, item, index) =>
      item.desktop > max.value ? { index, month: item.month, value: item.desktop } : max,
    { index: 0, month: series[0].month, value: series[0].desktop },
  );
